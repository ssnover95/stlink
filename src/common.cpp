...
static inline uint32_t read_flash_cr(stlink_t * sl)
{
    ...
    if (sl->flash_type == STLinkFlashType::F4)
        reg = FLASH_F4_CR;
    else if (sl->flash_type == STLinkFlashType::L4)
        reg = STM32L4_FLASH_CR;
    else if (sl->flash_type == STLinkFlashType::G0)
        reg = STM32G0_FLASH_CR;
    else if (sl->flash_type == STLinkFlashType::WB)
...
static void unlock_flash(stlink_t * sl)
{
    ...
    if (sl->flash_type == STLinkFlashType::F1_XL)
    {
...
static void set_flash_cr_per(stlink_t * sl)
{
    ...
    if (sl->flash_type == STLinkFlashType::G0)
        cr_reg = STM32G0_FLASH_CR;
    else if (sl->flash_type == STLinkFlashType::WB)
        cr_reg = STM32WB_FLASH_CR;
    else
...
int stlink_chip_id(stlink_t * sl, STLinkSTM32ChipId * chip_id)
{
    uint32_t raw_id;
    int ret = stlink_read_debug32(sl, 0xE0042000, &raw_id);
    *chip_id = static_cast<STLinkSTM32ChipId>(raw_id);
    if (ret == -1)
        return ret;

    if (*chip_id == STLinkSTM32ChipId::UNKNOWN)
    {
        ret = stlink_read_debug32(sl, 0x40015800, &raw_id); // Try Cortex M0 DBGMCU_IDCODE register address
        *chip_id = static_cast<STLinkSTM32ChipId>(raw_id);
    }
...
int stlink_load_device_params(stlink_t * sl)
{
    ...
    if (sl->chip_id == STLinkSTM32ChipId::F2)
    {
        uint32_t cpuid;
        stlink_read_debug32(sl, 0xE000ED00, &cpuid);
        if ((cpuid & 0xfff0) == 0xc240)
            sl->chip_id = STLinkSTM32ChipId::F4;
    }
    ...
    if (params->flash_type == STLinkFlashType::UNKNOWN)
    {
    ...
    if ((sl->chip_id == STLinkSTM32ChipId::L1_MEDIUM ||
         sl->chip_id == STLinkSTM32ChipId::L1_MEDIUM_PLUS) &&
        (flash_size == 0))
    {
        sl->flash_size = 128 * 1024;
    }
    else if (sl->chip_id == STLinkSTM32ChipId::L1_CAT2)
    {
        sl->flash_size = (flash_size & 0xff) * 1024;
    }
    else if (sl->chip_id == STLinkSTM32ChipId::L1_HIGH)
    {
    ...
    if (sl->chip_id == STLinkSTM32ChipId::F1_VL_MEDIUM_LOW && sl->flash_size < 64 * 1024)
    {
        sl->sram_size = 0x1000;
    }
    ...
    ILOG(
        "SRAM size: %#x bytes (%d KiB), Flash: %#x bytes (%d KiB) in pages of "
        "%u bytes\n",
        sl->sram_size, sl->sram_size / 1024, sl->flash_size,
        sl->flash_size / 1024, (unsigned int)sl->flash_pgsz);
    return 0;
}
...
static bool stlink_fread_ihex_writeline(struct stlink_fread_ihex_worker_arg * the_arg)
{
    ...
    if (the_arg->lba != (addr & 0xFFFF0000))
    { // segment changed
...
static bool stlink_fread_ihex_worker(void * arg, uint8_t * block, ssize_t len)
{
    ...
    {
        if (the_arg->buf_pos == sizeof(the_arg->buf))
        { // line is full
...
uint32_t calculate_L4_page(stlink_t * sl, uint32_t flashaddr)
{
    ...
    if (sl->chip_id == STLinkSTM32ChipId::L4 || sl->chip_id == STLinkSTM32ChipId::L496X ||
        sl->chip_id == STLinkSTM32ChipId::L4RX)
    {
...
{
    if ((sl->chip_id == STLinkSTM32ChipId::F2) || (sl->chip_id == STLinkSTM32ChipId::F4) ||
        (sl->chip_id == STLinkSTM32ChipId::F4_DE) ||
        (sl->chip_id == STLinkSTM32ChipId::F4_LP) ||
        (sl->chip_id == STLinkSTM32ChipId::F4_HD) ||
        (sl->chip_id == STLinkSTM32ChipId::F411RE) ||
        (sl->chip_id == STLinkSTM32ChipId::F446) ||
        (sl->chip_id == STLinkSTM32ChipId::F4_DSI) ||
        (sl->chip_id == STLinkSTM32ChipId::F72XXX) ||
        (sl->chip_id == STLinkSTM32ChipId::F412))
    {
    ...
    else if (sl->chip_id == STLinkSTM32ChipId::F7 || sl->chip_id == STLinkSTM32ChipId::F7XXXX)
    {
...
{
    if (sl->flash_type == STLinkFlashType::F4 || sl->flash_type == STLinkFlashType::L4)
    {
        ...
        if ((sl->chip_id == STLinkSTM32ChipId::L4) ||
            (sl->chip_id == STLinkSTM32ChipId::L43X) ||
            (sl->chip_id == STLinkSTM32ChipId::L46X) ||
            (sl->chip_id == STLinkSTM32ChipId::L496X))
        {
    ...
    else if (sl->flash_type == STLinkFlashType::L0)
    {
        uint32_t val;
        uint32_t flash_regs_base;
        if (sl->chip_id == STLinkSTM32ChipId::L0 || sl->chip_id == STLinkSTM32ChipId::L0_CAT5 ||
            sl->chip_id == STLinkSTM32ChipId::L0_CAT2 ||
            sl->chip_id == STLinkSTM32ChipId::L011)
        {
    ...
    else if (sl->flash_type == STLinkFlashType::WB || sl->flash_type == STLinkFlashType::G0)
    {
        uint32_t val;
        ...
        if (sl->flash_type == STLinkFlashType::WB)
        {
    ...
    else if ((sl->flash_type == STLinkFlashType::F0) ||
             ((sl->flash_type == STLinkFlashType::F1_XL) && (flashaddr < FLASH_BANK2_START_ADDR)))
    {
        /* wait for ongoing op to finish */
        wait_flash_busy(sl);
    ...
    else if ((sl->flash_type == STLinkFlashType::F1_XL) && (flashaddr >= FLASH_BANK2_START_ADDR))
    {
        /* wait for ongoing op to finish */
        wait_flash_busy(sl);
...
int stlink_erase_flash_mass(stlink_t * sl)
{
    ...
    if (sl->flash_type == STLinkFlashType::L0 ||
        sl->flash_type == STLinkFlashType::G0 || sl->flash_type == STLinkFlashType::WB)
    {
        /* erase each page */
...
int stm32l1_write_half_pages(stlink_t * sl, stm32_addr_t addr, uint8_t * base, uint32_t len, uint32_t pagesize)
{
    ...
    if (sl->chip_id == STLinkSTM32ChipId::L0 || sl->chip_id == STLinkSTM32ChipId::L0_CAT5 ||
        sl->chip_id == STLinkSTM32ChipId::L0_CAT2 || sl->chip_id == STLinkSTM32ChipId::L011)
    {
        flash_regs_base = STM32L0_FLASH_REGS_ADDR;
    }
    else
...
int stlink_write_flash(stlink_t * sl, stm32_addr_t addr, uint8_t * base, uint32_t len, uint8_t eraseonly)
{
    ...
    if ((sl->flash_type == STLinkFlashType::F4) || (sl->flash_type == STLinkFlashType::L4))
    {
            ...
            (sl->chip_id != STLinkSTM32ChipId::L496X))
        {
            ...
            {
                printf(
                    "STLINK V1 cannot read voltage, defaulting to 32-bit "
            ...
            else
            {
                ...
                {
                    printf(
                        "Target voltage (%d mV) too low for 32-bit flash, "
    ...
    else if (sl->flash_type == STLinkFlashType::L0)
    {
        ...
        stlink_write_debug32(sl, flash_regs_base + FLASH_PECR_OFF, val);
             ...
             (sl->flash_type == STLinkFlashType::F1_XL))
    {
    ...
    *begin = UINT32_MAX;
    ...
    { // parse file two times - first to find memory range, second - to fill it
        ...
        while (fgets(line, sizeof(line), file))
        {
            ...
            while (l > 0 && (line[l - 1] == '\n' || line[l - 1] == '\r'))
                --l; // trim EoL
            if ((l < 11) || (l == (sizeof(line) - 1)))
            { // line too short or long - wrong file format
...
int stlink_mwrite_flash(stlink_t * sl, uint8_t * data, uint32_t length, stm32_addr_t addr)
{
    ...
    err = stlink_write_flash(
        sl, addr, data, (num_empty == length) ? (uint32_t)length : (uint32_t)length - num_empty,
        num_empty == length);
    stlink_fwrite_finalize(sl, addr);
    return err;
}
...
int stlink_write_option_bytes(stlink_t * sl, stm32_addr_t addr, uint8_t * base, uint32_t len)
{
    ...
    if ((sl->chip_id != STLinkSTM32ChipId::G0X1) || (addr != STM32_G0_OPTION_BYTES_BASE))
    {
        ELOG(
            "Option bytes writing is currently only supported for the "
            "STM32G0\n");
        return -1;
    }
    ...
    if ((val & (1u << STM32G0_FLASH_CR_LOCK)))
    {
        ...
        {
            ELOG(
                "Flash unlock failed! System reset required to be able to "
                "unlock it again!\n");
            return -1;
        }
    }
    ...
    if ((val & (1 << STM32G0_FLASH_CR_OPTLOCK)))
    {
        ...
        {
            ELOG(
                "Options bytes unlock failed! System reset required to be able "
                "to unlock it again!\n");
            return -1;
        }
    }
