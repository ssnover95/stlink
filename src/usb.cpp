...
int _stlink_usb_reset(stlink_t * sl)
{
    ...
    return stlink_write_debug32(
        sl, STLINK_REG_AIRCR, STLINK_REG_AIRCR_VECTKEY | STLINK_REG_AIRCR_SYSRESETREQ);
}
...
static stlink_backend_t _stlink_usb_backend = {_stlink_usb_close,
...
static size_t stlink_probe_usb_devs(libusb_device ** devs, stlink_t ** sldevs[])
{
    ...
    while ((dev = devs[i++]) != NULL)
    {
        ...
        ret = libusb_get_string_descriptor_ascii(
            handle, desc.iSerialNumber, (unsigned char *)&serial, sizeof(serial));
        if (ret < 0)
            *serial = 0;
