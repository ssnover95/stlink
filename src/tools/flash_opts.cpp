...
int flash_get_opts(struct flash_opts * o, int ac, char ** av)
{
    ...
    while (ac >= 1)
    {
        ...
        else if (strcmp(av[0], "--serial") == 0 ||
                 starts_with(av[0], "--serial="))
        {
            const char * serial;
            ...
            for (size_t k = 0; j >= 0 && k < sizeof(o->serial); ++k, j -= 2)
            {
                ...
                o->serial[length - k] = (uint8_t)strtol(buffer, nullptr, 16);
            }

            serial_specified = true;
        }
        else if (strcmp(av[0], "--format") == 0 ||
                 starts_with(av[0], "--format="))
        {
            const char * format;
    ...
    while (ac >= 1)
    { // looks like for stlinkv1 the device name and command may be swaped - check both positions in all cases
        if (strcmp(av[0], "erase") == 0)
        {
            if (o->cmd != FLASH_CMD_NONE)
                return -1;
    ...
    switch (o->cmd)
    {
        ...
        case FLASH_CMD_WRITE:
            if (o->format == FLASH_FORMAT_BINARY)
            { // expect filename and addr
                if (ac != 2)
                    return -1;
            ...
            else if (o->format == FLASH_FORMAT_IHEX)
            { // expect filename
                if (ac != 1)
                    return -1;
    ...
    if (serial_specified && o->devname != nullptr)
        return -1; // serial not supported for v1
