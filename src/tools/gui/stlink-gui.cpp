...
static void stlink_gui_set_sensitivity(STlinkGUI * gui, gboolean sensitivity)
{
    ...
    gtk_widget_set_sensitive(GTK_WIDGET(gui->export_button),
                             sensitivity && (gui->sl != NULL));
}
...
static void mem_view_init_headers(GtkTreeView * view)
{
    ...
    gtk_tree_view_insert_column_with_attributes(view, -1, "Address", renderer,
                                                "text", 0, /* column */
                                                NULL);
    for (i = 0; i < 4; i++)
    {
        ...
        renderer = gtk_cell_renderer_text_new();
        gtk_tree_view_insert_column_with_attributes(view, -1, label, renderer,
                                                    "text", (i + 1), /* column */
                                                    NULL);
        g_free(label);
    }
...
static void mem_view_add_as_hex(GtkListStore * store, GtkTreeIter * iter, gint column, guint32 value)
{
    gchar * hex_str;
...
static void mem_view_add_buffer(
    GtkListStore * store, GtkTreeIter * iter, guint32 address, guchar * buffer, gint len)
{
    guint32 * word;
    gint i, step;
    gint column = 0;
...
static guint32 hexstr_to_guint32(const gchar * str, GError ** err)
{
    ...
    {
        g_set_error(err, g_quark_from_string("hextou32"), 1,
                    "Invalid hexstring");
        return UINT32_MAX;
    }
    if (end_ptr == str)
    {
        g_set_error(err, g_quark_from_string("hextou32"), 2,
                    "Invalid hexstring");
        return UINT32_MAX;
    }
    return val;
...
static gpointer stlink_gui_populate_filemem_view(gpointer data)
{
    ...
    if (g_str_has_suffix(gui->filename, ".hex"))
    {
        ...
        {
            stlink_gui_set_info_error_message(
                gui, "Cannot interpret the file as Intel-HEX");
        }
    ...
    else
    {
        ...
        for (off = 0; off < (gint)gui->file_mem.size; off += MEM_READ_SIZE)
        {
            ...
            if (g_input_stream_read(G_INPUT_STREAM(input_stream), &buffer,
                                    n_read, NULL, &err) == -1)
            {
                stlink_gui_set_info_error_message(gui, err->message);
                g_error_free(err);
...
static void mem_jmp(GtkTreeView * view, GtkEntry * entry, guint32 base_addr, gsize size, GError ** err)
{
    GtkTreeModel * model;
    guint32 jmp_addr;
    GtkTreeIter iter;
...
static void devmem_jmp_cb(GtkWidget * widget, gpointer data)
{
    ...
    mem_jmp(gui->devmem_treeview, gui->devmem_jmp_entry, gui->sl->flash_base,
            gui->sl->flash_size, &err);
...
static void filemem_jmp_cb(GtkWidget * widget, gpointer data)
{
    ...
    mem_jmp(gui->filemem_treeview, gui->filemem_jmp_entry, 0, gui->file_mem.size, &err);
...
static void stlink_gui_open_file(STlinkGUI * gui)
{
    ...
    dialog = gtk_file_chooser_dialog_new(
        "Open file", gui->window, GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel",
...
static gpointer stlink_gui_write_flash(gpointer data)
{
    ...
    if (stlink_mwrite_flash(gui->sl, gui->file_mem.memory,
                            (uint32_t)gui->file_mem.size, gui->sl->flash_base) < 0)
    {
...
static void flash_button_cb(GtkWidget * widget, gpointer data)
{
    ...
    if (result == GTK_RESPONSE_OK)
    {
        ...
        else
        {
            ...
            {
                stlink_gui_set_info_error_message(gui,
                                                  "Binary overwrites flash");
            }
            else
            {
                stlink_gui_set_sensitivity(gui, FALSE);
...
static void export_button_cb(GtkWidget * widget, gpointer data)
{
    ...
    GtkWidget * dialog;
    dialog = gtk_file_chooser_dialog_new(
        "Save as", gui->window, GTK_FILE_CHOOSER_ACTION_SAVE, "_Cancel",
        GTK_RESPONSE_CANCEL, "_Open", GTK_RESPONSE_ACCEPT, NULL);
...
static void notebook_switch_page_cb(GtkNotebook * notebook, GtkWidget * widget, guint page_num, gpointer data)
{
    STlinkGUI * gui;
    (void)notebook;
    (void)widget;
                            ...
                            gpointer data)
{
    ...
    gtk_drag_finish(context, TRUE, gdk_drag_context_get_suggested_action(context) == GDK_ACTION_MOVE,
                    timestamp);
}
...
void stlink_gui_init_dnd(STlinkGUI * gui)
{
    ...
    gtk_drag_dest_set(GTK_WIDGET(gui->window), GTK_DEST_DEFAULT_ALL,
                      target_list, G_N_ELEMENTS(target_list), GDK_ACTION_COPY);
...
static void stlink_gui_build_ui(STlinkGUI * gui)
{
    ...
    devmem_store = gtk_list_store_new(5, G_TYPE_STRING, G_TYPE_STRING,
    ...
    filemem_store = gtk_list_store_new(5, G_TYPE_STRING, G_TYPE_STRING,
                                       G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
    ...
    gui->flash_dialog_cancel = GTK_BUTTON(
        gtk_builder_get_object(builder, "flash_dialog_cancel_button"));
