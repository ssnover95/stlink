...
{
    puts(
        "stlinkv1 command line: ./st-flash [--debug] [--reset] [--format "
        "<format>] [--flash=<fsize>] {read|write} /dev/sgX <path> <addr> "
        "<size>");
    puts("stlinkv1 command line: ./st-flash [--debug] /dev/sgX erase");
    puts(
        "stlinkv2 command line: ./st-flash [--debug] [--reset] [--serial "
        "<serial>] [--format <format>] [--flash=<fsize>] {read|write} <path> "
        "<addr> <size>");
    puts(
        "stlinkv2 command line: ./st-flash [--debug] [--serial <serial>] "
        "erase");
    puts(
        "stlinkv2 command line: ./st-flash [--debug] [--serial <serial>] "
        "reset");
    ...
    puts(
        "                       fsize: Use decimal, octal or hex by prefix "
        "0xXXX for hex, optionally followed by k=KB, or m=MB (eg. "
        "--flash=128k)");
    puts(
        "                       Format may be 'binary' (default) or 'ihex', "
        "although <addr> must be specified for binary format only.");
...
int main(int ac, char ** av)
{
    ...
    if (o.devname != nullptr) /* stlinkv1 */
        sl = stlink_v1_open(o.log_level, 1);
    else /* stlinkv2 */
        sl = stlink_open_usb(static_cast<ugly_loglevel>(o.log_level), 1,
                             (char *)o.serial);
    ...
    if (o.cmd == FLASH_CMD_WRITE) /* write */
    {
        ...
        {
            err = stlink_parse_ihex(o.filename, stlink_get_erased_pattern(sl),
                                    &mem, &size, &o.addr);
            if (err == -1)
            {
