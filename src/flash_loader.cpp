...
static const uint8_t loader_code_stm32f4[] = {
    ...
    0x62, 0xb1, 0x04, 0x68, 0x0c, 0x60,

    0xdc, 0x89, 0x14, 0xf0, 0x01, 0x0f, 0xfb, 0xd1, 0x00, 0xf1, 0x04,
    0x00, 0x01, 0xf1, 0x04, 0x01, 0xa2, 0xf1, 0x01, 0x02, 0xf1, 0xe7,
...
static const uint8_t loader_code_stm32f4_lv[] = {
    ...
    0x08, 0x4b, 0x62, 0xb1, 0x04, 0x78, 0x0c, 0x70,

    0xdc, 0x89, 0x14, 0xf0, 0x01, 0x0f, 0xfb, 0xd1, 0x00, 0xf1, 0x01,
    0x00, 0x01, 0xf1, 0x01, 0x01, 0xa2, 0xf1, 0x01, 0x02, 0xf1, 0xe7,
...
static int loader_v_dependent_assignment(stlink_t * sl,
                                         ...
                                         size_t * loader_size,
                                         const uint8_t * high_v_loader,
                                         size_t high_v_loader_size,
                                         const uint8_t * low_v_loader,
                                         size_t low_v_loader_size)
{
    int retval = 0;
...
int stlink_flash_loader_write_to_sram(stlink_t * sl, stm32_addr_t * addr, size_t * size)
{
    ...
    if (sl->chip_id == STLinkSTM32ChipId::L1_MEDIUM ||
        sl->chip_id == STLinkSTM32ChipId::L1_CAT2 ||
        sl->chip_id == STLinkSTM32ChipId::L1_MEDIUM_PLUS ||
        sl->chip_id == STLinkSTM32ChipId::L1_HIGH ||
        sl->chip_id == STLinkSTM32ChipId::L152_RE || sl->chip_id == STLinkSTM32ChipId::L011 ||
        sl->chip_id == STLinkSTM32ChipId::L0 || sl->chip_id == STLinkSTM32ChipId::L0_CAT5 ||
        sl->chip_id == STLinkSTM32ChipId::L0_CAT2)
    { /* stm32l */
        loader_code = loader_code_stm32l;
        loader_size = sizeof(loader_code_stm32l);
    }
    else if (sl->core_id == STM32VL_CORE_ID || sl->chip_id == STLinkSTM32ChipId::F1_MEDIUM ||
             sl->chip_id == STLinkSTM32ChipId::F3 ||
             sl->chip_id == STLinkSTM32ChipId::F3_SMALL ||
             sl->chip_id == STLinkSTM32ChipId::F303_HIGH ||
             sl->chip_id == STLinkSTM32ChipId::F37x || sl->chip_id == STLinkSTM32ChipId::F334)
    {
        loader_code = loader_code_stm32vl;
        loader_size = sizeof(loader_code_stm32vl);
    }
    else if (sl->chip_id == STLinkSTM32ChipId::F2 || sl->chip_id == STLinkSTM32ChipId::F4 ||
             ...
             sl->chip_id == STLinkSTM32ChipId::F410 ||
             sl->chip_id == STLinkSTM32ChipId::F411RE ||
             sl->chip_id == STLinkSTM32ChipId::F412 ||
             sl->chip_id == STLinkSTM32ChipId::F413 || sl->chip_id == STLinkSTM32ChipId::F446)
    {
        int retval;
        retval = loader_v_dependent_assignment(
            sl, &loader_code, &loader_size, loader_code_stm32f4, sizeof(loader_code_stm32f4),
    ...
    else if (sl->core_id == STM32F7_CORE_ID || sl->chip_id == STLinkSTM32ChipId::F7 ||
             sl->chip_id == STLinkSTM32ChipId::F7XXXX ||
             sl->chip_id == STLinkSTM32ChipId::F72XXX)
    {
        int retval;
        retval = loader_v_dependent_assignment(
            sl, &loader_code, &loader_size, loader_code_stm32f7, sizeof(loader_code_stm32f7),
    ...
    else if (sl->chip_id == STLinkSTM32ChipId::F0 || sl->chip_id == STLinkSTM32ChipId::F04 ||
             sl->chip_id == STLinkSTM32ChipId::F0_CAN ||
             sl->chip_id == STLinkSTM32ChipId::F0_SMALL ||
             sl->chip_id == STLinkSTM32ChipId::F09X)
    {
        loader_code = loader_code_stm32f0;
        loader_size = sizeof(loader_code_stm32f0);
    }
    else if ((sl->chip_id == STLinkSTM32ChipId::L4) ||
             (sl->chip_id == STLinkSTM32ChipId::L41X) ||
    ...
    {
        ELOG(
            "unknown coreid, not sure what flash loader to use, aborting! "
            "coreid: %x, chipid: %x\n",
                            ...
                            size_t size)
{
    ...
    if ((sl->flash_type == STLinkFlashType::F0) || (sl->flash_type == STLinkFlashType::F1_XL))
    {
        count = size / sizeof(uint16_t);
        if (size % sizeof(uint16_t))
            ++count;
    }
    else if (sl->flash_type == STLinkFlashType::F4 || sl->flash_type == STLinkFlashType::L0)
    {
    ...
    if ((sl->flash_type == STLinkFlashType::F1_XL) && (target >= FLASH_BANK2_START_ADDR))
    {
