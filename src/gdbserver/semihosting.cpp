...
static int open_mode_flags[12] = {O_RDONLY,
                                  O_RDONLY | O_BINARY,
                                  O_RDWR,
                                  O_RDWR | O_BINARY,
...
int do_semihosting(stlink_t * sl, uint32_t r0, uint32_t r1, uint32_t * ret)
{
    ...
    switch (r0)
    {
        case SEMIHOST_SYS_OPEN:
        {
            ...
            /* Add the trailing zero that is not counted in the length argument
             * (see ARM semihosting specification)
             */
            name_len += 1;
            ...
            {
                DLOG(
                    "Semihosting SYS_OPEN error: name buffer size is too big "
                    "%d\n",
                    name_len);
                *ret = -1;
                return -1;
            ...
            {
                DLOG(
                    "Semihosting SYS_OPEN error: cannot allocate name "
                    "buffer\n");
                *ret = -1;
                return -1;
            }
        ...
        case SEMIHOST_SYS_REMOVE:
        {
            ...
            {
                DLOG(
                    "Semihosting SYS_REMOVE error: cannot allocate name "
                    "buffer\n");
                *ret = -1;
                return -1;
            }
