...
static bool execute_test(const struct Test * test)
{
    ...
    for (char * tok = strtok(cmd_line, " "); tok; tok = strtok(nullptr, " "))
    {
...
static struct Test tests[] = {
    ...
    {"--debug --reset read /dev/sg0 test.bin 0x80000000 0x1000",
     0,
     {.cmd = FLASH_CMD_READ,
      .devname = "/dev/sg0",
      .serial = {0},
    ...
    {"--debug --reset write /dev/sg0 test.bin 0x80000000",
     0,
     {.cmd = FLASH_CMD_WRITE,
      .devname = "/dev/sg0",
      .serial = {0},
    ...
    {"/dev/sg0 erase",
     ...
     {.cmd = FLASH_CMD_ERASE,
      ...
      .format = FLASH_FORMAT_BINARY}},
    {"--debug --reset read test.bin 0x80000000 0x1000",
     0,
     {.cmd = FLASH_CMD_READ,
      .devname = nullptr,
      .serial = {0},
      ...
      .log_level = DEBUG_LOG_LEVEL,
      .format = FLASH_FORMAT_BINARY}},
    {"--debug --reset write test.bin 0x80000000",
     0,
     {.cmd = FLASH_CMD_WRITE,
      .devname = nullptr,
      .serial = {0},
    ...
    {"erase",
     ...
     {.cmd = FLASH_CMD_ERASE,
      ...
      .log_level = STND_LOG_LEVEL,
      .format = FLASH_FORMAT_BINARY}},
    {"--debug --reset --format=ihex write test.hex",
     0,
     {.cmd = FLASH_CMD_WRITE,
      .devname = nullptr,
      .serial = {0},
    ...
    {"--serial A1020304 erase",
     0,
     {.cmd = FLASH_CMD_ERASE,
      .devname = nullptr,
      .serial = {0, 0, 0, 0, 0, 0, 0, 0, 0xA1, 0x02, 0x03, 0x04},
      .filename = nullptr,
      .addr = 0,
      .size = 0,
      .reset = 0,
